...
class SegSynthesisBlock(torch.nn.Module):
    ...
    def __init__(self,
        ...
        resolution,                         # Resolution of this block.
        img_channels,                       # Number of output color channels.
        seg_channels,                       # Number of output segmentation channels.
        is_last,                            # Is this the last block?
        architecture        = 'skip',       # Architecture: 'orig', 'skip', 'resnet'.
        resample_filter     = [1,3,3,1],    # Low-pass filter to apply when resampling activations.
        conv_clamp          = None,         # Clamp the output of convolution layers to +-X, None = disable clamping.
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        if is_last or architecture == 'skip':
            ...
            self.toseg = ToRGBLayer(
                out_channels, seg_channels, w_dim=w_dim,
                conv_clamp=conv_clamp, channels_last=self.channels_last,
                groups=self.groups, mode=self.mode)
            self.num_toseg += 1
    ...
    def forward(self, x, img, seg, ws, force_fp32=False, fused_modconv=None, add_on=None, block_noise=None, disable_rgb=False, **layer_kwargs):
        misc.assert_shape(ws, [None, self.num_conv + self.num_torgb, self.w_dim])
        w_iter = iter(ws.unbind(dim=1))
        dtype = torch.float16 if self.use_fp16 and not force_fp32 else torch.float32
        memory_format = torch.channels_last if self.channels_last and not force_fp32 else torch.contiguous_format
        ...
        if self.is_last or self.architecture == 'skip':
            if not disable_rgb:
                y = x if add_on is None else x + add_on
                y = self.torgb(y, w_shared, fused_modconv=fused_modconv)
                y = y.to(dtype=torch.float32, memory_format=torch.contiguous_format)
                img = img.add_(y) if img is not None else y
            else:
                img = None
        ...
        # ToSEG.
        if seg is not None:
            if seg.size(-1) * 2 == x.size(-1):
                if (self.upsample_mode == 'bilinear_all') or (self.upsample_mode == 'bilinear_ada'):
                    seg = F.interpolate(seg, scale_factor=2, mode='bilinear', align_corners=True)
                else:
                    seg = upfirdn2d.upsample2d(seg, self.resample_filter)   # this is upsampling. Not sure about details and why they do this..
            elif seg.size(-1) == x.size(-1):
                pass
            else:
                raise NotImplementedError
        ...
        if self.is_last or self.architecture == 'skip':
            if not disable_rgb:
                y_seg = x if add_on is None else x + add_on
                y_seg = self.toseg(y_seg, w_shared, fused_modconv=fused_modconv)
                y_seg = y_seg.to(dtype=torch.float32, memory_format=torch.contiguous_format)
                seg = seg.add_(y_seg) if seg is not None else y_seg
            else:
                seg = None

        assert x.dtype == dtype
        assert img is None or img.dtype == torch.float32
        assert seg is None or seg.dtype == torch.float32
        return x, img, seg
...
class Encoder(torch.nn.Module):
    def __init__(self, size, n_latents, w_dim=512, add_dim=0, input_dim=3, **unused):
        super().__init__()
        ...
        convs = [Conv2dLayer(input_dim, channels[size], 1)]
    ...
    def forward(self, input):
        ...
        if self.add_dim == 0:
            out = out.view(len(input), self.n_latents, self.w_dim)
            return out
        pws, pcm = out[:, :-2], out[:, -2:]
...
class HybridEncoder(torch.nn.Module):
    def __init__(self, size, n_latents_app, n_latents_geo, w_dim=512, add_dim=0, input_img_dim=3, input_seg_dim=19, **unused):
        super().__init__()
        ...
        self.n_latents_app = n_latents_app
        self.n_latents_geo = n_latents_geo
        convs = [Conv2dLayer(input_img_dim, channels[size], 1)]
        ...
        self.convs_img = torch.nn.Sequential(*convs)
        self.projector_img = EqualConv2d(in_channel, self.n_latents_app*self.w_dim + add_dim, 4, padding=0, bias=False)

        convs = [Conv2dLayer(input_seg_dim, channels[size], 1)]
        ...
        self.convs_seg = torch.nn.Sequential(*convs)
        self.projector_seg = EqualConv2d(in_channel, self.n_latents_geo*self.w_dim, 4, padding=0, bias=False)

    def forward(self, img, seg):
        batch = img.size(0)
        out_img = self.convs_img(img)
        out_img = self.projector_img(out_img)
        out_seg = self.convs_seg(seg)
        out_seg = self.projector_seg(out_seg)
        if self.add_dim == 0:
            out_img = out_img.view(batch, self.n_latents_app, self.w_dim)
            out_seg = out_seg.view(batch, self.n_latents_geo, self.w_dim)
            return torch.cat([out_seg, out_img], 1)
        
        pws_img, pcm = out_img[:, :-2], out_img[:, -2:]
        pws_img = pws_img.view(batch, self.n_latents_app, self.w_dim)
        pcm = pcm.view(batch, self.add_dim)
        pws_seg = out_seg.view(batch, self.n_latents_geo, self.w_dim)

        return torch.cat([pws_seg, pws_img], 1), pcm


@persistence.persistent_class
class MultiViewHybridEncoder(torch.nn.Module):
    def __init__(self, size, n_latents_app, n_latents_geo, w_dim=512, add_dim=0, input_img_dim=3, input_seg_dim=19, num_view=3, **unused):
        super().__init__()
        ...
        linear_img = [FullyConnectedLayer(in_channel//2, 256)]
        for i in range(2):
            linear_img.append(FullyConnectedLayer(256, 256))
        linear_img.append(FullyConnectedLayer(256,  self.n_latents_app*self.w_dim+add_dim))
        self.projector_img = torch.nn.Sequential(*linear_img)
        ...
        linear_seg = [FullyConnectedLayer(in_channel//2, 256)]
        for i in range(2):
            linear_seg.append(FullyConnectedLayer(256, 256))
        linear_seg.append(FullyConnectedLayer(256,  self.n_latents_geo*self.w_dim))
        self.projector_seg = torch.nn.Sequential(*linear_seg)

    def forward(self, img, seg, num_view=4):
        if num_view == 1:
            batch = img.size(0)
            out_img = self.convs_img(img)
        else:
            assert num_view == self.num_view
            batch = img.size(0) // self.num_view
        
        out_img = self.convs_img(img)
        if num_view == self.num_view:
            out_img = self.view_fusion_module(out_img, batch)
        else:
            out_img = out_img.split(out_img.shape[1]//2, 1)[1]
            out_img = out_img.flatten(start_dim=1)
            
        out_img = self.projector_img(out_img)

        out_seg = self.convs_seg(seg)
        if num_view == self.num_view:
            out_seg = self.view_fusion_module(out_seg, batch)
        else:
            out_seg = out_seg.split(out_seg.shape[1]//2, 1)[1]
            out_seg = out_seg.flatten(start_dim=1)
    ...
    def view_fusion_module(self, x, batch):
        x = torch.stack(x.split(batch, 0), 0)
        sigma, feat = x.split(x.shape[2] // 2, 2)
        sigma, feat = sigma.flatten(start_dim=2), feat.flatten(start_dim=3)
        ...
        return feat_weighted.flatten(start_dim=1)
